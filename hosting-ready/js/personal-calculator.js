...
function renderHealth(matrix) {
  healthBody.innerHTML = healthRows.map((row, index) => {
    ...
    const indexLabel = index < healthRows.length - 1 ? index + 1 : '';

    return `
      <tr class="health-row-${index + 1}">
        <td class="health-index" aria-label="${indexLabel ? `Чакра ${indexLabel}` : 'Сума'}">${indexLabel}</td>
        <td data-label="Зона"><strong>${row[0]}</strong><br><span>${row[1]}</span></td>
        <td data-label="Фізика">${values[0]}</td>
        <td data-label="Енергія">${values[1]}</td>
        <td data-label="Емоції">${values[2]}</td>
      </tr>
    `;
  }).join('');
}
