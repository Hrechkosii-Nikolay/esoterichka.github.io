...
if (!function_exists('curl_init')) {
  error_log('Social click tracking: PHP cURL extension is unavailable.');
...
$posted_platform = clean_value($platform, 80);
...
$telegram_status = curl_getinfo($ch, CURLINFO_RESPONSE_CODE);
$telegram_error = curl_error($ch);
curl_close($ch);

$telegram_data = json_decode((string)$telegram_response, true);
$telegram_ok = $telegram_status === 200 && !empty($telegram_data['ok']);

if (!$telegram_ok) {
  $description = $telegram_data['description'] ?? ($telegram_error ?: 'Unknown Telegram API error');
  error_log("Social click tracking failed (HTTP {$telegram_status}): {$description}");
